...
const ApplicationStatusPage: React.FC = () => {
  ...
  const handleApprove = () => {
    ...
    const event = new CustomEvent('petlove-push-notification', {
      detail: {
        title: '安心爪爪庇护所',
        message: '🎉 恭喜！您的领养申请已通过。Buddy 已经迫不及待要见到你了！点击查看领养证。',
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <header className="flex items-center justify-between p-4 sticky top-0 z-10 bg-background-light/95 backdrop-blur-sm">
        <button onClick={() => navigate('/home')} className="flex items-center justify-center w-10 h-10 rounded-full active:bg-black/5">
          <span className="material-symbols-outlined">home</span>
        </button>
        <h1 className="text-lg font-bold flex-1 text-center pr-10">申请状态</h1>
      </header>

      <main className="flex-1 px-5 pt-2 pb-32">
        <div className="mb-8">
          <div className="flex items-center gap-4 p-4 rounded-xl bg-white shadow-card border border-slate-100">
            <div className="relative w-20 h-20 shrink-0">
              <div 
                className="w-full h-full rounded-full bg-cover bg-center border-2 border-white shadow-sm" 
            ...
            <div>
              ...
              <p className="text-slate-400 text-sm font-medium">金毛寻回犬 • 2岁</p>
        ...
        <div className="relative pl-2">
          <div className="absolute left-[27px] top-4 bottom-10 w-[2px] bg-slate-100"></div>
          
          <div className="relative grid grid-cols-[50px_1fr] gap-x-3 mb-8">
            <div className="relative flex flex-col items-center">
              <div className="z-10 flex items-center justify-center w-8 h-8 rounded-full bg-primary text-white shadow-md ring-4 ring-background-light">
                <span className="material-symbols-outlined text-[18px]">check</span>
              </div>
              <div className="absolute top-8 bottom-[-32px] w-[2px] bg-primary"></div>
            </div>
            <div className="pt-1">
              <p className="text-base font-bold">提交申请</p>
              <p className="text-sm text-slate-400 mt-0.5">2023年9月28日</p>
            </div>
          </div>
          ...
          <div className="relative grid grid-cols-[50px_1fr] gap-x-3 mb-8">
            <div className="relative flex flex-col items-center">
              ...
              <div className={`absolute top-8 bottom-[-32px] w-[2px] ${status === 'approved' ? 'bg-primary' : 'bg-slate-100'}`}></div>
            </div>
            <div className="pt-1">
              <p className="text-base font-bold">初步审核</p>
              <p className="text-sm text-slate-400 mt-0.5">于2023年9月30日通过</p>
            </div>
          </div>

          <div className="relative grid grid-cols-[50px_1fr] gap-x-3 mb-8">
            <div className="relative flex flex-col items-center">
              <div className={`z-10 flex items-center justify-center w-8 h-8 rounded-full ${status === 'approved' ? 'bg-primary text-white' : 'bg-orange-500 text-white'} shadow-md ring-4 ring-background-light`}>
            ...
            <div className={`pt-1 ${status !== 'approved' ? 'cursor-pointer' : ''}`} onClick={status !== 'approved' ? handleApprove : undefined}>
              <p className="text-base font-bold">家庭回访</p>
              <p className="text-sm text-slate-400 mt-0.5">{status === 'approved' ? '已完成' : '点击模拟通过家庭回访'}</p>
            </div>
          </div>

          <div className="relative grid grid-cols-[50px_1fr] gap-x-3">
            <div className="relative flex flex-col items-center">
              <div className={`z-10 flex items-center justify-center w-8 h-8 rounded-full ${status === 'approved' ? 'bg-primary text-white shadow-lg ring-4 ring-background-light scale-110' : 'bg-white border-2 border-slate-200 text-slate-300 ring-4 ring-background-light'}`}>
            ...
            <div className={`pt-1 ${status !== 'approved' ? 'opacity-60' : ''}`}>
              <p className="text-base font-bold">最终审批</p>
              <p className="text-sm text-slate-400 mt-0.5">{status === 'approved' ? '恭喜！领养申请已通过' : '预计时间：待定'}</p>
            </div>
          </div>
        </div>
      </main>

      <div className="fixed bottom-0 left-0 right-0 p-5 bg-gradient-to-t from-background-light to-transparent pt-10 pb-8 flex flex-col gap-3 max-w-md mx-auto z-20">
        <button 
          onClick={() => status === 'approved' ? navigate(`/success/${id}`) : navigate('/messages')} 
          className="w-full flex items-center justify-center gap-2 rounded-full h-14 bg-primary text-white font-bold shadow-lg shadow-primary/20"
        >
        ...
        <button onClick={() => navigate('/home')} className="w-full h-10 text-slate-400 text-sm font-semibold">返回首页</button>
      </div>
    </div>
  );
};
